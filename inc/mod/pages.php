...
function mod_login($redirect = false) {
	...
	if (isset($_POST['login'])) {
		...
		if (!isset($_POST['username'], $_POST['password']) || $_POST['username'] == '' || $_POST['password'] == '') {
			...
			$query = prepare("INSERT INTO modlogins VALUES (:name, :ip, :time, 0)");
			...
			$query = prepare("INSERT INTO modlogins VALUES (:name, :ip, :time, 1)");
			$query->bindValue(":name", $_POST['username']);
			$query->bindValue(":ip", $_SERVER['REMOTE_ADDR']);
			$query->bindValue(":time", time());
			$query->execute() or error(db_error($query));
...
function mod_bans($full="") {
	...
	mod_page(_('Ban list'), 'mod/ban_list.html', array(
		...
		'token_json' => make_secure_link_token('bans' . $full . '.json'),
		'full_list' => $full
	));
}

function mod_bans_json($full="", $token) {
        global $config, $mod;
	...
	if($config['JSON_ban_compression']){
		...
		Bans::stream_json(false, false, !hasPermission($config['mod']['view_banstaff']), $mod['boards'], $full == "-full");
	}
	else{
		Bans::stream_json(false, false, !hasPermission($config['mod']['view_banstaff']), $mod['boards'],  $full == "-full");
	}
...
function mod_whitelist_tokens(){
	global $config;
	global $mod;
	$query = prepare("SELECT * FROM ``whitelist_tokens``") or error(db_error());
	$query->execute();
	$token_list = $query->fetchAll(PDO::FETCH_CLASS);
	mod_page(_('White List Tokens'), 'mod/whitelist_tokens.html', array(
		'token' => make_secure_link_token('alter-wl-token'),
		'wl_tokens' => $token_list
	));
}

function mod_alter_whitelist_token(){
	global $config;
	if(trim($_POST["owner"]) && isset($_POST["create"])){
		$query = prepare("INSERT INTO whitelist_tokens VALUES(NULL, :token, :owner)");
		$token = substr(crypt( time() . trim($_POST["owner"]) . rand (PHP_INT_MIN , PHP_INT_MAX), "$2y$07$" . $config["secure_wl_token_salt"]), -31);
		$query->bindValue(":token", phoneticEncoding($token));
		$query->bindValue(":owner", trim($_POST["owner"]));
		$query->execute() or error(db_error($query));
	} else if(trim($_POST["revoke-code"]) && isset($_POST["revoke"])){
		$query = prepare("DELETE FROM whitelist_tokens WHERE token=:token");
		$query->bindValue(":token", trim($_POST["revoke-code"]));
		$query->execute() or error(db_error($query));
	} else{
		error("Fields not set");
	}
	header('Location: ?/wl-tokens', true, $config['redirect_http']);
}

function mod_whitelist() {
	global $config;
	global $mod;
	...
	if (isset($_POST['unwhitelist'])) {
		if (!hasPermission($config['mod']['remove_whitelist']))
			error($config['error']['noaccess']);

		$unwhitelist = array();
		foreach ($_POST as $name => $unused) {
			if (preg_match('/^white_(\d+)$/', $name, $match))
				$unwhitelist[] = $match[1];
		}
		if (isset($config['mod']['unban_limit']) && $config['mod']['unban_limit'] && count($unwhitelist) > $config['mod']['unban_limit'])
			error(sprintf($config['error']['toomanyunban'], $config['mod']['unban_limit'], count($unwhitelist)));
		foreach ($unwhitelist as $id) {
			Bans::unwhitelist($id, true, $mod['boards'], true);
		}
    rebuildThemes('bans');
		header('Location: ?/whitelist', true, $config['redirect_http']);
		return;
	}
	mod_page(_('White list'), 'mod/white_list.html', array(
		'mod' => $mod,
		'boards' => json_encode($mod['boards']),
		'token' => make_secure_link_token('whitelist'),
		'token_json' => make_secure_link_token('white.json')
	));
}

function mod_whitelist_json() {
        global $config, $mod;
	...
	if($config['JSON_ban_compression']){
		...
		Bans::stream_whitelist_json(false, false, !hasPermission($config['mod']['view_banstaff']), $mod['boards']);
	}
	else{
		Bans::stream_whitelist_json(false, false, !hasPermission($config['mod']['view_banstaff']), $mod['boards']);
	}
...
function mod_ban_appeals() {
	...
	if (isset($_POST['appeal_id']) && (isset($_POST['unban']) || isset($_POST['hole'])  || isset($_POST['whitelist-register'])  ||isset($_POST['deny'] ))) {
		if (!hasPermission($config['mod']['ban_appeals']))
			error($config['error']['noaccess']);
		if((isset($_POST['hole']) || isset($_POST['whitelist-register'])  ) && !(isset($_POST['wl-pattern']) &&
				@preg_match($_POST['wl-pattern'], null) !== false)){
			error($config['error']['nowhitelistpattern']);
		}
		...
		if (isset($_POST['unban'])) {
			modLog('Accepted ban appeal #' . $ban['id'] . ' for ' . $ban['mask'] . ' - Message: ' . $ban['message']);
			Bans::delete($ban['ban_id'], true);
			Hazuki::evaluateAppeal($ban['message'], 0);
			query("DELETE FROM ``ban_appeals`` WHERE `id` = " . $ban['id']) or error(db_error());
		} else if (isset($_POST['hole'])){
			modLog('Accepted hole appeal #' . $ban['id'] . ' for ' . $ban['mask'] . ' - Message: ' . $ban['message']);
			Bans::whitelist($ban['mask'], $_POST['wl-pattern'], false, $mod['id']);
			Hazuki::evaluateAppeal($ban['message'], 0);
			query("DELETE FROM ``ban_appeals`` WHERE `id` = " . $ban['id']) or error(db_error());
		}else if (isset($_POST['whitelist'])){
			modLog('Accepted whitelist appeal #' . $ban['id'] . ' for ' . $ban['mask'] . ' - Message: ' . $ban['message']);
			Bans::whitelist($ban['mask'], $_POST['wl-pattern'], false, $mod['id']);
			Hazuki::evaluateAppeal($ban['message'], 0);
			query("DELETE FROM ``ban_appeals`` WHERE `id` = " . $ban['id']) or error(db_error());
		} else {
			modLog('Denied ban appeal #' . $ban['id'] . ' for ' . $ban['mask'] . ' - Message: ' . $ban['message']);
			if(!isset($_POST['deny-reason']) || !$_POST['deny-reason']){
				Hazuki::evaluateAppeal($ban['message'], 1);
			}
			query("UPDATE ``ban_appeals`` SET `denied` = 1 " .
				(isset($_POST['deny-reason']) ? ', `denial_reason` = "' . $_POST['deny-reason'] . '"' : "")
...
function mod_move_reply($originBoard, $postID) {
	...
	if (isset($_POST['board'])) {
		...
		if(isset($_POST['target_thread']) && trim($_POST['target_thread']) != ""){
			...
			Hazuki::rebuildCatalog($targetBoard);
		}
		else{
			// build new thread
			$post['id'] = $newID;
		...
		Hazuki::decacheThread($postID, $originBoard);
		...
		deletePost($postID, true, true, true, true);
		buildIndex();
		Hazuki::rebuildOverboard();
		Hazuki::send(true);
	...
	if (true || isset($_POST['board'])) {
		...
		if(isset($post['thread'])){
			$post['body_nomarkup'] = preg_replace(
				...
				'[link-ommited]' ,
				$post['body_nomarkup'] . "\n\n'''Deleted by  " . $mod['username'] . " from >>>/$originBoard/" . $post['thread'] ." Post No. $postID" . " from thread " . $post['thread'] . "'''");
				$post['body'] = preg_replace(
					'/[a-z0-9\-_]*?\.([a-z0-9])[\/\?\#a-z0-9]*\b/i',
					'[link-ommited]' ,
					$post['body'] . "<br/><br/><strong>Deleted by  " . $mod['username'] . " from <a href='/$originBoard/res/" . $post['thread'] ."#" . $post['thread'] . "'>&gt;&gt;&gt;/$originBoard/". $post['thread'] ."</a> Post No. $postID" . " from thread " . $post['thread'] . "</strong>");
		} else{
			$post['body_nomarkup'] = preg_replace(
				'/[a-z0-9\-_]*?\.([a-z0-9])[\/\?\#a-z0-9]*\b/i',
				'[link-ommited]' ,
				$post['body_nomarkup'] . "\n\n'''Deleted by  " . $mod['username'] . " from >>>/$originBoard/" . $post['thread'] ." Post No. $postID" . " (OP)" . "'''");
				$post['body'] = preg_replace(
					'/[a-z0-9\-_]*?\.([a-z0-9])[\/\?\#a-z0-9]*\b/i',
					'[link-ommited]' ,
					$post['body'] . "<br/><br/><strong>Deleted by  " . $mod['username'] . " from <a href='/$originBoard/" ."'>&gt;&gt;&gt;/$originBoard/". $post['thread'] ."</a> Post No. $postID" . " (OP)" . "</strong>");
		}
		...
		foreach ($replies as &$post) {
			...
			$post['body'] = preg_replace(
						'/[a-z0-9\-_]*?\.([a-z0-9])[\/\?\#a-z0-9]*\b/i',
								'[link-ommited]' ,
								$post['body'] . "<br/><br/><strong>Moved by followup</strong>");
			if ($post['has_file']) {
			...
			if (!empty($post['tracked_cites'])) {
				...
				foreach ($post['tracked_cites'] as $cite) {
					$query = prepare(sprintf('SELECT id, thread FROM posts_%s WHERE id=:id UNION SELECT id, thread FROM posts_%s WHERE id=:t_id', $board['uri'], $cite[0]));
					$query->bindValue(':id', (int)$newPostID);
					$query->bindValue(':t_id', (int)$cite[1]);
					$query->execute() or error(db_error($query));
					$host_objects = $query->fetchAll(PDO::FETCH_ASSOC);
					if(count($host_objects) < 2){
						error("Citation post/target does not exist");
					}
					$host = $host_objects[0]["thread"] ? $host_objects[0]["thread"] : $host_objects[0]["id"];
					$target_host = $host_objects[1]["thread"] ? $host_objects[1]["thread"] : $host_objects[1]["id"];
					$insert_rows[] = '(' .
						$pdo->quote($board['uri']) . ', ' . (int)$newPostID . ', ' .
						$pdo->quote($cite[0]) . ', ' . (int)$cite[1] . ',' . $host . ',' . $target_host . ')';
				}
		...
		Hazuki::decachePost($postID, $originBoard);
		Hazuki::decachePost($newID, $targetBoard);

		Hazuki::rebuildCatalog($originBoard);
		Hazuki::rebuildCatalog($targetBoard);
...
function mod_move($originBoard, $postID) {
	...
	if (isset($_POST['board'])) {
		...
		if ($_POST['target_thread']) {
			...
			$query->bindValue(':id', $_POST['target_thread']);
			$query->execute() or error(db_error($query)); // If it fails, thread probably does not exist
			$test_post = $query->fetch();
			if(!$test_post){
					error("Merge thread target does not exist");
			}
			$post['op'] = false;
			$post['thread'] = $_POST['target_thread'];
		}
		...
		$op = $post;
		$post["body"] = $post["body_nomarkup"];
		markup($post["body"], true);
		if(isset($_POST['target_thread']) && trim($_POST['target_thread']) != ""){
			//needed for link_for redirect
			$newID = $_POST['target_thread'];
		}
		else{
			$newID = post($post);
			// build new thread
		...
		$query = prepare(sprintf('SELECT * FROM ``posts_%s`` WHERE `thread` = :id '. (isset($_POST['target_thread']) && trim($_POST['target_thread']) ? ' OR `id`=:id ' : '') .' ORDER BY `id`', $originBoard));
		$query->bindValue(':id', $postID, PDO::PARAM_INT);
		$query->execute() or error(db_error($query));
		...
		if ($shadow) {
			...
			Hazuki::decacheHome();
			Hazuki::decachePost($postID, $originBoard);
			Hazuki::decachePost($_POST['target_thread'], $targetBoard);
			buildIndex();
			Hazuki::send(true);
			// trigger themes
...
function mod_ban_hash($board, $post_no, $redirect=true){
	global $config;
	if(!isset($_POST['reason'])){
			$args = array(
				'post' => $post_no,
				'board' => $board,
				'token' => make_secure_link_token($board . '/hash/' . $post_no)
			);

			mod_page(_('Hash Ban'), 'mod/hash_form.html', $args);
	}else{
		...
		$files[0]->hash = 'banned';
		$thumb = $files[0]->thumb_path;

		$output = shell_exec_error("blockhash " . $thumb);
		...
		$query = prepare(sprintf("INSERT INTO hashlist (id, hash, reason)
		SELECT * FROM (SELECT NULL, :filehash,:reason) AS filehash
		WHERE NOT EXISTS (
			SELECT hash FROM hashlist WHERE hash = :filehash
		) LIMIT 1" , $board ));
		$query->bindValue(':filehash', $hash);
		$query->bindValue(':reason', $_POST['reason']);
		$query->execute() or error(db_error($query));
...
function mod_ban_post($board, $delete, $hash, $post, $token = false) {
	global $config, $mod;
	...
	if (isset($_POST['new_ban'], $_POST['reason'], $_POST['length'], $_POST['board'])) {
		...
		if(isset($_POST['hash']) && (int) $_POST['hash'] ){
			//mod_ban_hash($board, $post, 0, false);
		}
...
function mod_edit_post($board, $edit_raw_html, $postID) {
	...
	if (isset($_POST['name'], $_POST['email'], $_POST['subject'], $_POST['body'])) {
		...
		Hazuki::rebuildThread($postID, $board);
		Hazuki::rebuildCatalog($board);
		buildIndex();
		Hazuki::rebuildHome();
		Hazuki::rebuildSummary();
...
function mod_delete($board, $post, $redirect_on_nerf=true) {
	...
	$thread_id = deletePost($post, true, true, false, true);
	...
	Hazuki::rebuildProperties($thread_id, $board);
	Hazuki::rebuildSummary();
	Hazuki::rebuildHome();
	Hazuki::decacheHome();
	Hazuki::send(true);
...
function mod_deletefile($board, $post, $file) {
	...
	if (!hasPermission($config['mod']['deletefile'], $board))
		error($config['error']['noaccess']);
	// Delete file
	if(preg_match('/^[a-zA-Z]+[a-zA-Z0-9._]+$/', $file)){
		deleteFile($post, TRUE, 0);
	}else{
...
function mod_spoiler_image($board, $fn_str = "spoiler", $post, $file) {
	global $config, $mod;
...
function mod_unspoiler_image($board, $fn_str = "spoiler", $post, $file){
	global $config, $mod;
...
function mod_deletebyip($boardName, $post, $global = false) {
	...
	while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
		...
		if(!nerf_mod_check($board['uri'], $post['id'], $redirect=false))
		 	deletePost($post['id'], true, true, false, true);
	...
	$boards_to_build = array();
	foreach ($threads_to_rebuild as $_board => $_threads) {
		openBoard($_board);
		if(!in_array($_board, $boards_to_build)){
			$boards_to_build[] = $_board;
		}
	...
	foreach($boards_to_build as $build_board){
		Hazuki::rebuildCatalog($build_board);
	}
	Hazuki::rebuildOverboard();
	Hazuki::rebuildSummary();
	Hazuki::rebuildHome();
	...
	if (isset($_POST['rebuild'])) {
		...
		foreach ($boards as $board) {
			...
			if (isset($_POST['rebuild_posts'])) {
				...
				while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
					$log[] = '<strong>' . sprintf($config['board_abbreviation'], $board['uri']) . '</strong>: Rebuilding reply #' . $post['id'];
					rebuildPost($post['id']);
				}
			}
...
function mod_recent_posts($lim) {
	...
	foreach ($mod_boards as $board) {
		$query .= sprintf('SELECT id,thread,subject,email,name,trip,capcode,body,body_nomarkup,time,bump,files,num_files,filehash,password,ip,sticky,locked,cycle,sage,embed,slug,wl_token, %s AS `board` FROM ``posts_%s`` UNION ALL ', $pdo->quote($board['uri']), $board['uri']);
	}
