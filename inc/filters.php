...
class Filter {
	...
	public function action($is_wl=false) {
		global $board;
		global $config;
			...
			switch($this->action) {
				...
				case 'captcha':
					...
					if($config['flood_recaptcha']  && $this->post['captype'] == 'recaptcha'){
						...
						captchouli(isset($this->message) ? $this->message .
							"<hr style='width:40%'/>
							<iframe class='captchouli' id='captchouli-frame' src='/captcha'></iframe>
							<script>document.getElementById('captchouli-frame').onload = setCaptchouliListeners</script><br/>
							<form style='display:none' action='/post.php' method='post'>
							<textarea style='width: 95%;' name='captchouli' id='captchouli-code' placeholder='Captcha code goes here' required></textarea>
							<input name='reference' type='hidden' value='".  $holding_id . "'>
							<input name='release' type='hidden' value='submit'>
							<input name='board' type='hidden' value='".  $this->post['board'] . "'><br/>
							<input type='submit' id='captchouli-submit'>
							<br/><hr/>
							</form><hr/>Submit the Captcha to proceed!" : 'Error Message Captchouli .', $holding_id);
					}
					else{
...
function do_filters(array $post) {
	...
	$is_wl = count(Bans::findInWhitelist($_SERVER['REMOTE_ADDR']), true) != 0;
