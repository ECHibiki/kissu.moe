		...
		public function build($mod = false) {
			...
			foreach($boards as &$_board) {
				if(in_array($_board['uri'], explode(' ', $this->settings['exclude'])))
					continue;
				$query .= sprintf("SELECT id,subject,email,name,trip,capcode,body,body_nomarkup,time,bump,files,num_files,password,sticky,locked,cycle,sage,embed,slug, '%s' AS `board` FROM ``posts_%s`` WHERE `thread` IS NULL UNION ALL ", $_board['uri'], $_board['uri']);
			}
			...
			if($this->settings['global_message']){
				$config['global_message'] = $this->settings['global_message'];
			}
